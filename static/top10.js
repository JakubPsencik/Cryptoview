...
class SimpleEarn {
	...
	async displaySimpleEarnTable() {
	
		try {
			let response = await fetch(this.url);
			response.json().then(async (r) => {

				let markers = await this.checkViewData(r)
				
				var arr = r.slice(2,12)
				var compound = []
				var fixed = []
				...
				this.createSimpleEarnTable(compound, fixed, markers, "saving_staking_table")
			});
	...
	async createSimpleEarnTable(compound, fixed, markers, tableName) {
		...
		let counter = 0;
		for(let i = 0; i < 10; i++) {
			this.addSimpleEarnTableRow(markers[i].name, compound[i], fixed[i], [markers[i].daily,markers[i].weekly,markers[i].monthly], markers[i].base, markers[i].quote, tableName)
			counter+=1;
			if(counter == 3) {
				counter = 0
			}
	...
	async checkViewData(response) {
	
		var daily = response[1];
		var weekly = response[13];
		var monthly = response[25];
		var markers = [];
		var currentMarkerRow = {};
	...
	async addSimpleEarnTableRow(name, compound, fixed, markers, base, quote, tableName) {
