async function rb_displayBinanceIndexAssetData(url) {
	...
	try {
		...
		response.json().then(async (data) => {
			...
			var divName = `rebalance_binance_index_asset_row_div_${indexAssetRowDivNumber}`
			var div = document.createElement('div');
			div.id = divName
			div.classList.add("binance_index_asset_row_div");
			document.getElementById("rebalance_binance_index_asset_main_div").appendChild(div);
			indexAssetRowDivNumber += 1;
			for (let i = 0; i < data.length; i++) {

				rb_createAssetDiv(data[i], div);
				divsRenderedCounter += 1;

				if(divsRenderedCounter == divsToRender) {
					div = document.createElement('div');
					divName = `rebalance_binance_index_asset_row_div_${indexAssetRowDivNumber}`;
					div.id = divName
					div.classList.add("binance_index_asset_row_div");
					document.getElementById("rebalance_binance_index_asset_main_div").appendChild(div);
		...
		console.log("error in: rebalance_binance_index_asset.js")
		console.log(error);
	}
}
	...
	nameDiv.addEventListener("click", () => {
		...
		if(data.coins.length < 11) {
			for(let i = 0; i < data.coins.length; i++) {
				rb_createIndexAssetCoinRow(data.coins[i].coin, parseFloat(data.coins[i].allocation)*100);
			}
			...
			const auto_invest_calc_div = document.getElementById("rebalance-widget");
			auto_invest_calc_div.style.display = "block"; 

			const rebalance_settings_div = document.getElementById("rebalance_settings_div");
			rebalance_settings_div.style.display = "block"; 
...
<<<<<<< HEAD
		document.getElementById("rebalance_page_binance_index_asset").appendChild(overlay);
		document.getElementById("rebalance_page_binance_index_asset").appendChild(allocationDetailsDiv);
=======
...
<<<<<<< HEAD
		document.getElementById("rebalance_page_binance_index_asset").appendChild(overlay);
		document.getElementById("rebalance_page_binance_index_asset").appendChild(indexDetailsDiv);
=======
		document.getElementById("page_auto_invest_calc").appendChild(overlay);
...
async function createInfoDivs(indexDetailsDiv, data) {
	...
	infoDiv1.id = `rebalance_binance_index_asset_index_detailsInfoDiv1`;
	...
	infoDiv2.id = `rebalance_binance_index_asset_index_detailsInfoDiv2`;
	...
	infoDiv3.id = `rebalance_binance_index_asset_index_detailsInfoDiv3`;
	...
	infoDiv4.id = `rebalance_binance_index_asset_index_detailsInfoDiv4`;
